...
- En el árbol del DOM, el nodo raíz es el nodo `document`, que representa todo el documento HTML, es decir, árbol DOM de la página o, más concretamente, la página de la pestaña del navegador donde nos encontramos.
...
- Mientras que los nodos de tipo `Node` son los nodos de texto y atributos (<div class="dot" style="background-color: #96f2d7;"></div> y <div class="dot" style="background-color: #ffec99;"></div>).

::: ok
Es importante recordar que todos los nodos de tipo `Element` son también nodos de tipo `Node`, pero no todos los nodos de tipo `Node` son nodos de tipo `Element`.
:::
...
- Los _event listeners_ se pueden añadir a cualquier elemento del DOM, como un botón, un enlace, un campo de texto, etc.
  - Funcionando "publicando" un evento que se puede escuchar y manejar mediante una función _callback_.
...
- Este tipo de eventos son particularmente útiles para procesar formularios.
...
let fin_2024 = new Date(2024, 12, 31, 11, 59, 59);
```
...
- A diferencia de las cookies, el `localStorage` permite almacenar información de forma más sencilla y eficiente, por default, el alcance del `localStorage` es el dominio y el protocolo, por lo que los datos se mantienen incluso después de cerrar el navegador.
